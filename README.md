...
```json
{
  "mcpServers": {
    "docpilot": {
      "command": "/path/to/docpilot/.venv/bin/python",
      "args": [
        "/path/to/docpilot/mcp_server.py",
...
```bash
git clone <this-repo> /opt/docpilot
cd /opt/docpilot
...
Each project gets its own ChromaDB collection (namespaced by `--project`). The same docpilot installation supports multiple projects simultaneously.
...
# Both stored in docpilot/.chroma_db/ with separate collections
```
...
| `--chroma-dir` | no | `docpilot/.chroma_db/` | Override ChromaDB storage location |
